...
@Component({
  selector: 'page-categoryCode',
  templateUrl: 'categoryCode.html',
  providers: [Http]
})

export class CategoryCodePage {
  ...
  constructor(
            ...
            )
            {
              ...
              this.passedCode = this.navParams.get('code')
            }
  ...
  ionViewDidLoad() {
    ...
    this.itemApi.getItems().then(data => {
      loader.dismiss();
      this.items = data;
      this.items = this.items.filter(item => item.code == this.passedCode);
    });
  }
