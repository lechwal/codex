...
export class Category3Page {
  ...
  constructor(
            ...
            {
              //this.category3 = this.navParams.data;
              this.passedcategory3 = this.navParams.get('category');
            }
  ...
  ionViewDidLoad() {
    ...
    this.ItemApicLois.getItems().then(data => {
      loader.dismiss();
      this.items = data;
      this.items = this.items.filter(item => item.category == this.passedcategory3);
    });
